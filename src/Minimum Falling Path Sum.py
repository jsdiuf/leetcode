...
class Solution:
    ...
    def minFallingPathSum(self, A):
        ...
        def helper(r, c, sm, ans):
            if r == rc - 1:
                ans[0] = min(ans[0], sm)
            else:
                if c > 0:
                    helper(r + 1, c - 1, sm + A[r + 1][c - 1], ans)
                helper(r + 1, c, sm + A[r + 1][c], ans)
                if c < rc - 1:
                    helper(r + 1, c + 1, sm + A[r + 1][c + 1], ans)

        for i in range(rc):
            helper(0, i, A[0][i], ans)
        return ans[0]
...
s = Solution()
print(s.minFallingPathSum2([[1, 2, 3], [4, 5, 6], [7, 8, 9]]))
print(s.minFallingPathSum2([[-19, -1, -96, 48, -94, 36, 16, 55, -42, 37, -59, 6, -32, 96, 95, -58, 13, -34, 94, 85],
